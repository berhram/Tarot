...
interface CardInteractor : CardDetailsUseCase, CardsUseCase, CardsByKeywordUseCase,
    ...
    class Base(
        ...
        override suspend fun cachedCards(): List<CardDomain> =
            repository.cachedCards().map { combineToCardDomain(it) }

        override suspend fun cards(): List<CardDomain> =
            repository.cards().map { combineToCardDomain(it) }
        ...
        override suspend fun cardById(id: String): CardDomain =
            combineToCardDomain(repository.card(id))
    }
}